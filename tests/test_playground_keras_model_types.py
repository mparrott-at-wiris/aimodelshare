...
def test_keras_model_submission(model_name, model_factory, shared_playground, iris_data):
    ...
    try:
        ...
        if 'onnx' in error_str or 'conversion' in error_str:
            pytest.skip(f"Skipping {model_name} submission A due to ONNX conversion issue: {e}")
    ...
    try:
        ...
        # Skip if error is due to ONNX conversion issue
        if 'onnx' in error_str or 'conversion' in error_str:
            pytest.skip(f"Skipping {model_name} submission B due to ONNX conversion issue: {e}")
