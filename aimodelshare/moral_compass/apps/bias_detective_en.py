...
def create_bias_detective_en_app(theme_primary_hue: str = "indigo"):
    with gr.Blocks(theme=gr.themes.Soft(primary_hue=theme_primary_hue), css=css) as demo:
        ...
        with gr.Column(visible=False) as main_app_col:
            ...
            for i, mod in enumerate(MODULES):
                with gr.Column(
                    ...
                    if i in QUIZ_CONFIG:
                        ...
                        # Explicit Submit button and feedback area
                        submit_btn = gr.Button("Submit answer 🧭", variant="primary", elem_classes=["quiz-submit"])
                        feedback = gr.HTML("")
                        
                        quiz_wiring_queue.append((i, radio, submit_btn, feedback))
            ...
            # --- WIRING: QUIZ LOGIC ---
            for mod_id, radio_comp, submit_btn, feedback_comp in quiz_wiring_queue:
                ...
                def quiz_logic_wrapper(
                    ...
                    if not ans:
                        return (
                            gr.update(),
                            gr.update(),
                            "<div class='hint-box' style='border-color:var(--color-accent);'>"
                            "👉 Pick an option above, then press \"Submit answer 🧭\".</div>",
                            task_list,
                        )
