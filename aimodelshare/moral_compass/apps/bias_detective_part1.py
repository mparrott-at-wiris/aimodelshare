    ...
    with gr.Blocks(theme=gr.themes.Soft(primary_hue=theme_primary_hue), css=css) as demo:
        ...
        for i in range(len(MODULES)):
            ...
            if i < len(MODULES) - 1:
                ...
                def create_next_nav(user, tok, team, tasks, curr=curr_col, nxt=next_col, next_idx=i + 1):
                    ...
                    return dash_html, navigate_next()

                def wrapper_next(user, tok, team, tasks, curr=curr_col, nxt=next_col, next_idx=i + 1):
                    data, _ = ensure_table_and_get_data(user, tok, team, tasks)
                    dash_html = render_top_dashboard(data, next_idx)
                    return dash_html
